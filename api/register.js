module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rolFinal = ['admin','superadmin'].includes(rol) ? rol : 'admin';
    ...
    const r = await fetch(`${SB_URL}/rest/v1/usuarios`, {
      ...
      body: JSON.stringify({ nombre, email, password_hash, telefono: telefono || null, rol: rolFinal })
    });
    ...
    return res.status(200).json({ id: d[0].id, nombre: d[0].nombre, email: d[0].email, rol: d[0].rol });
  } catch (e) {
